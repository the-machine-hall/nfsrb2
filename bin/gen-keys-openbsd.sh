...
Copyright (C) 2014-2021 Frank Scheiner
...
if [ ! -d "$_rootOfFileSystem" ]; then
	...
	# Format of "/etc/nfsrb_openbsd_version" is "<VERSION> (<BUILDDATE>)"
	_openbsdVersionTarget=$( cat "$_rootOfFileSystem/etc/nfsrb_openbsd_version" | cut -d ' ' -f 1 )
